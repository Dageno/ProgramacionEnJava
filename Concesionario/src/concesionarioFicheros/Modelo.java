...
public enum Modelo {
	...
	public static Modelo[] getModelos(Marca marca){
		int i = 0;
		
		ArrayList<Modelo> modelosSize = new ArrayList<Modelo>();
		
		for(Modelo modelo : VALUES)
			if(modelo.getMarca() == marca)
				modelosSize.add(modelo);
		
		Modelo[] modeloPorMarca = new Modelo[modelosSize.size()];
		for(Modelo modelo : modelosSize)
			modeloPorMarca[i++] = modelo ;
		
		return modeloPorMarca;
